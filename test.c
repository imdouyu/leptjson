...
static void test_access_number() {
  ...
  EXPECT_EQ_DOUBLE(2021.0816, lept_get_number(&v));
  lept_free(&v);
}
...
static void test_stringify_array() {
  TEST_ROUNDTRIP("[]");
  TEST_ROUNDTRIP("[null,false,true,123,\"abc\",[1,2,3],\"helloworld\"]");
}
...
static void test_parse_expect_value() {
  ...
  TEST_ERROR(LEPT_PARSE_EXPECT_VALUE, " ");
  TEST_ERROR(LEPT_PARSE_EXPECT_VALUE, "{\"a\":{}, \"key\":");
}
...
static void test_parse_miss_comma_or_square_bracket() {
  ...
  TEST_ERROR(LEPT_PARSE_MISS_COMMA_OR_SQUARE_BRACKET, "[[1,2],  [1]");
}
...
static void test_parse_miss_key() {
  ...
  TEST_ERROR(LEPT_PARSE_MISS_KEY, "{\"a\":1,{\"   key   \" : \"  value  \",  }");
}
